...
Inoltre, anche se non riportato come caso d'uso, ogni categoria di cliente inseribile possiede l'azione di salvataggio, che va ad inserire i dati nel database AS400.\\
...
\begin{itemize}
	...
	\item \textbf{Post-condizione}:la nuova attività contenente le informazioni inserite viene salvata nel sistema ed è disponibile all'utente.
\end{itemize}
...
\begin{itemize}
	...
	\item \textbf{Post-condizione}:la nuova opportunità contenente le informazioni inserite viene salvata nel sistema ed è disponibile all'utente.
\end{itemize}

\section{UC 3\_G - Visualizzazione dettagli}
\begin{figure}[h]
	\centering
	\includegraphics[height = 8 cm]{/usecase/funzionalità-estese}
...
Per ogni categoria di utenti sono disponibili le seguenti funzionalità, non descritte nel dettaglio se non accompagnate dall'indicazione del caso d'uso corrispondente:
\begin{itemize}
	\item \textbf{Salva};
	\item \textbf{Modifica};
	\item \textbf{Annulla modifiche};
	\item \textbf{Elimina};
	\item \textbf{Invia mail}: [UC]; %TODO
	\item \textbf {Invia fax}: [UC];
	\item \textbf{Avvisa}.
\end{itemize}	 
